...
def _apply_migration(c):
    with c.cursor() as cur:
        cur.execute(
            f"DROP TABLE IF EXISTS {TABLE_PREFIX}audit, {TABLE_PREFIX}config, {TABLE_PREFIX}releases CASCADE")
        for f in sorted(_MIG_DIR.glob("*.up.sql")):   # 001 建表,按序应用
